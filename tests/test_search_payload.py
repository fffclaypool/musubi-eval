from musubi_eval.application.run_scenario import build_documents_payload, search_payload
from musubi_eval.domain.models import Document, SearchParam


def test_search_payload_basic():
    param = SearchParam(name="p", k=10)
    result = search_payload("hello", param, None)
...
def test_search_payload_extra_filter_only():
    param = SearchParam(name="p", k=10)
    result = search_payload("hello", param, {"status": "active"})
...
def test_search_payload_both_filters_query_wins_on_conflict():
    ...
    result = search_payload("hello", param, {"category": "tech"})
    assert result == {
        "text": "hello",
        "k": 10,
...
def test_build_documents_payload_basic():
    ...
    result = build_documents_payload(docs)
    assert result == [
        {"id": "d1", "text": "hello"},
...
def test_build_documents_payload_empty():
    assert build_documents_payload([]) == []
